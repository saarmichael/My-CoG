...
const ImageSelector = () => {
    ...
    const [imageParams, setImageParams] = useState<{ azimuth: number, elevation: number, distance: number }>(
    ...
    useEffect(() => {
        ...
        fetchParams().then((lists) => {
            ...
            refAzimuth.current!.value = lists.azi_list[0].toString();
            refElevation.current!.value = lists.ele_list[0].toString();
            refDistance.current!.value = lists.dist_list[0].toString();
        });
    }, []);
