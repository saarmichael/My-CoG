...
def graph_video(data, channels, fs, duration, video_name, epoch_duration=0.5):
    ...
    for epoch in range(n_epochs):
        ...
        for i in range(n_channels - 1):
            for j in range(i + 1, n_channels):
                f, Cxy = coherence(
                    data[start:end, i], data[start:end, j], fs, window, epoch_duration
                )
                # get only beta band
                Cxy = Cxy[3:17]
                ...
                con[j, i, epoch] = con[
                    i, j, epoch
                ]  # the coherence measure is symmetric
    ...
    for fra in range(n_epochs):
        ...
        annotation_trace = go.Scatter(
            x=[], y=[], text=[], mode="text", textposition="bottom center"
        )
        ...
        # Create a figure
        fig = go.Figure(
            data=edge_traces + [node_trace, annotation_trace],
            layout=go.Layout(
                showlegend=False,
                height=600,
                width=600,
