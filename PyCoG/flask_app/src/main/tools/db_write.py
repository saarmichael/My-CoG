...
class DB_manager:
    def delete_old_calculations(self):
        one_month_ago = datetime.now() - timedelta(weeks=4)
        old_calculations = Calculation.query.filter(
            Calculation.time < one_month_ago
        ).all()
    ...
    def write_calculation(self, file_name, url, data, created_by):
        self.delete_old_calculations()
        calculation = {
            "file_name": file_name,
            "url": url,
            "data": data,
    ...
    def write_user(self, username, data_dir, settings):
    ...
    def update_data_dir(self, username, new_data_dir):
    ...
    def write_settings(self, username, data):
        user = user_in_db(username, User.query)
        user.settings = data
        db.session.commit()
